...
;NAME DEFINITIONS FOR GLOBAL VARIABLES ALLOCATED TO REGISTERS
	.DEF _speed_blue=R4
	.DEF _speed_blue_msb=R5
	.DEF _speed_red=R6
	.DEF _speed_red_msb=R7
	.DEF _time=R8
	.DEF _time_msb=R9
...
; .FSTART _ext_int0_isr
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 0023     if (PIND.1 == 0)
	SBIC 0x10,1
	RJMP _0x3
; 0000 0024     {
; 0000 0025         if (speed_blue>=2 && speed_blue <=5)
	LDI  R30,LOW(2)
	LDI  R31,HIGH(2)
	CP   R4,R30
	CPC  R5,R31
...
; 0000 0029     }
; 0000 002A 
; 0000 002B     if (PIND.1 == 1)
_0x3:
	SBIS 0x10,1
	RJMP _0x7
; 0000 002C     {
; 0000 002D          if (speed_red>=4 && speed_red <=10)
	LDI  R30,LOW(4)
	LDI  R31,HIGH(4)
	CP   R6,R30
	CPC  R7,R31
...
; .FSTART _ext_int1_isr
	...
	ST   -Y,R30
; 0000 0037 
; 0000 0038 // PlaceI your code here
; 0000 0039 if (PIND.1 == 0)
	SBIC 0x10,1
	RJMP _0xB
; 0000 003A {
; 0000 003B 
; 0000 003C 
; 0000 003D         if (speed_blue>=1 && speed_blue <=4)
	LDI  R30,LOW(1)
	LDI  R31,HIGH(1)
	CP   R4,R30
	CPC  R5,R31
...
; 0000 0041 
; 0000 0042 }
; 0000 0043 if (PIND.1 == 1)
_0xB:
	SBIS 0x10,1
	RJMP _0xF
; 0000 0044 {
; 0000 0045         if (speed_red>=2 && speed_red <=8)
	LDI  R30,LOW(2)
	LDI  R31,HIGH(2)
	CP   R6,R30
	CPC  R7,R31
...
; 0000 0056     time ++;
	...
	MOVW R8,R30
; 0000 0057 
; 0000 0058     if (time % (speed_red) == 0) {
	MOVW R30,R6
	MOVW R26,R8
	CALL __MODW21U
	SBIW R30,0
	BRNE _0x13
; 0000 0059             led_red = ~led_red ;
	SBIS 0x1B,1
	RJMP _0x14
	CBI  0x1B,1
	RJMP _0x15
...
; 0000 005A             //time = 0;
; 0000 005B         }
; 0000 005C     if (time % (speed_blue)== 0){
_0x13:
	MOVW R30,R4
	MOVW R26,R8
	CALL __MODW21U
...
; 0000 005F         // time = 0;
; 0000 0060     }
; 0000 0061  /*  if (PORTD.3 == 1){
; 0000 0062         led_test = ~led_test;
; 0000 0063         if (time == speed_red)
; 0000 0064         {
; 0000 0065             led_red = ~ led_red;
; 0000 0066         } else {
; 0000 0067         // Place your code here
; 0000 0068         //if (time == speed_blue){
; 0000 0069             led_blue = ~ led_blue;
; 0000 006A         }
; 0000 006B         time = 0;
; 0000 006C     }
; 0000 006D  */
...
; 0000 0075 // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=Out Bit0=Out
; 0000 0076 DDRA=(0<<DDA7) | (0<<DDA6) | (0<<DDA5) | (0<<DDA4) | (0<<DDA3) | (0<<DDA2) | (1<<DDA1) | (1<<DDA0);
	LDI  R30,LOW(3)
	OUT  0x1A,R30
; 0000 0077 // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=0 Bit0=0
...
; 0000 007C DDRB=(0<<DDB7) | (0<<DDB6) | (0<<DDB5) | (0<<DDB4) | (0<<DDB3) | (1<<DDB2) | (0<<DDB1) | (0<<DDB0);
	LDI  R30,LOW(4)
	OUT  0x17,R30
...
; 0000 008E // Clock value: 0.977 kHz
; 0000 008F // Mode: Normal top=0xFF
; 0000 0090 // OC0 output: Disconnected
; 0000 0091 // Timer Period: 0.10035 s
; 0000 0092 TCCR0=(0<<WGM00) | (0<<COM01) | (0<<COM00) | (0<<WGM01) | (1<<CS02) | (0<<CS01) | (1<<CS00);
	LDI  R30,LOW(5)
	OUT  0x33,R30
; 0000 0093 TCNT0=0x9E;
	LDI  R30,LOW(158)
	OUT  0x32,R30
; 0000 0094 OCR0=0x00;
	LDI  R30,LOW(0)
...
; 0000 00B8 TIMSK=(0<<OCIE2) | (0<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (0<<TOIE1) | (0<<OCIE0) | (1<<TOIE0);
	LDI  R30,LOW(1)
	OUT  0x39,R30
; 0000 00B9 
...
; 0000 00BB // INT0: On
; 0000 00BC // INT1: On
; 0000 00BD // INT1 Mode: Falling Edge
; 0000 00BE // INT2: Off
; 0000 00BF GICR|=(1<<INT1) | (1<<INT0) | (0<<INT2);
	IN   R30,0x3B
	ORI  R30,LOW(0xC0)
	OUT  0x3B,R30
; 0000 00C0 MCUCR=(1<<ISC11) | (0<<ISC10) | (1<<ISC01) | (0<<ISC00);
	LDI  R30,LOW(10)
	OUT  0x35,R30
; 0000 00C1 MCUCSR=(0<<ISC2);
	LDI  R30,LOW(0)
	OUT  0x34,R30
; 0000 00C2 GIFR=(1<<INTF1) | (1<<INTF0) | (0<<INTF2);
	LDI  R30,LOW(192)
	OUT  0x3A,R30
; 0000 00C3 
